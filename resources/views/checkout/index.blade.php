<x-front-layout>
    <section class="max-w-5xl mx-auto py-12 px-6">
        <h2 class="text-2xl font-semibold mb-8">Finalizar pedido</h2>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                    ...
                    @foreach ($cart as $id => $item)
                        ...
                        <li class="flex justify-between items-start py-3">
                            ...
                            <div class="text-right">
                                <p class="text-sm text-gray-800">${{ number_format($item['price'] * $item['quantity'], 2) }}</p>
                                @if ($showBs)
                ...
                <div class="mt-5 pt-5 border-t border-gray-100">
                    @if (session('success'))
                        <p class="text-xs text-emerald-600 mb-2">{{ session('success') }}</p>
                    @endif
                    @if (session('error'))
                        <p class="text-xs text-red-500 mb-2">{{ session('error') }}</p>
                    @endif
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                ...
                <form action="{{ route('checkout.process') }}" method="POST">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" name="rif" value="{{ old('rif', $user->rif) }}" required
                                ...
                                class="w-full border rounded-lg p-2.5 text-sm @error('rif') border-red-400 @else border-gray-200 @enderror">
                            @error('rif')
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" required
                                ...
                                class="w-full border rounded-lg p-2.5 text-sm @error('phone') border-red-400 @else border-gray-200 @enderror">
                            @error('phone')
                                <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="mb-5">
                        <label class="block text-sm text-gray-500 mb-1">Dirección de envío</label>
                        <textarea name="address" rows="3" required placeholder="Calle, ciudad, estado..."
                            class="w-full border rounded-lg p-2.5 text-sm resize-none @error('address') border-red-400 @else border-gray-200 @enderror">{{ old('address', $user->address) }}</textarea>
                        @error('address')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-2 mb-6">
                        @foreach (['card' => 'Tarjeta', 'paypal' => 'PayPal', 'transfer' => 'Transferencia'] as $value => $label)
                            <label class="cursor-pointer">
                                <input type="radio" name="payment" value="{{ $value }}" class="sr-only peer"
                                    {{ old('payment', 'card') === $value ? 'checked' : '' }}>
                                <div class="text-center text-sm py-2.5 border border-gray-200 rounded-lg
                    ...
                    <button type="submit"
                        class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-xl transition text-base">
                        Confirmar pedido →
                    </button>

                    <p class="text-center text-xs text-gray-300 mt-3 flex items-center justify-center gap-1">
